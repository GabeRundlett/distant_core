...
int main() {
    ...
    auto injectable_filepath_remote_buffer = process.virtual_alloc_unique(str.size() + 1);
    ...
    auto load_library_address = GetProcAddress(kernel32_dll_handle, "LoadLibraryA");
    ...
    auto thread_handle = CreateRemoteThread(
        ...
        std::bit_cast<void *>(injectable_filepath_remote_buffer.address), 0, nullptr);
    if (!thread_handle) {
